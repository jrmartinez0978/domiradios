...
@section('title', 'Ciudades - Directorio de Emisoras')
        ...
        @foreach($genres as $genre)
            <div class="bg-white p-4 rounded-lg shadow-md flex flex-col items-center">
                <a href="{{ route('ciudades.show', ['slug' => $genre->slug]) }}">
                    ...
                    <img src="{{ $genre->img ? Storage::url($genre->img) : asset('path/to/default-image.jpg') }}"
                         alt="{{ $genre->name }}"
                ...
                <a href="{{ route('ciudades.show', ['slug' => $genre->slug]) }}" class="mt-4 bg-green-500 text-white px-6 py-2 rounded-full hover:bg-green-600 transition">
                    Ver emisoras
                </a>
            </div>
        @endforeach
