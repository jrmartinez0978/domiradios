        ...
        <form method="POST" action="{{ route('admin.themes.update', $theme) }}" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-2xl border border-surface-300 shadow-sm p-6 space-y-5 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <x-admin.form-toggle name="is_active" label="Activo" :checked="(bool) $theme->isActive" />
                    <x-admin.form-toggle name="is_single_theme" label="Tema Unico" :checked="(bool) $theme->is_single_theme" />
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-surface-300 shadow-sm p-6 space-y-5 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" name="grad_start_color" id="grad_start_color" value="{{ old('grad_start_color', $theme->grad_start_color ?: '#667eea') }}" class="h-10 w-14 rounded-lg border border-surface-300 cursor-pointer">
                            <input type="text" id="grad_start_color_text" value="{{ old('grad_start_color', $theme->grad_start_color ?: '#667eea') }}" class="glass-input flex-1" maxlength="7" oninput="document.getElementById('grad_start_color').value = this.value" onchange="document.getElementById('grad_start_color').value = this.value">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" name="grad_end_color" id="grad_end_color" value="{{ old('grad_end_color', $theme->grad_end_color ?: '#764ba2') }}" class="h-10 w-14 rounded-lg border border-surface-300 cursor-pointer">
                            <input type="text" id="grad_end_color_text" value="{{ old('grad_end_color', $theme->grad_end_color ?: '#764ba2') }}" class="glass-input flex-1" maxlength="7" oninput="document.getElementById('grad_end_color').value = this.value" onchange="document.getElementById('grad_end_color').value = this.value">
                        </div>
                ...
                <x-admin.form-select name="grad_orientation" label="Orientacion del Gradiente" :options="$orientationOptions" :value="$theme->grad_orientation" />

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1.5">Vista Previa</label>
                    <div id="gradient-preview" class="h-20 rounded-xl border border-surface-300" style="background: linear-gradient({{ $theme->grad_orientation }}deg, {{ $theme->grad_start_color ?: '#667eea' }}, {{ $theme->grad_end_color ?: '#764ba2' }})"></div>
                </div>
            </div>

            <div class="bg-white rounded-2xl border border-surface-300 shadow-sm p-6 space-y-5 mb-6">
                <h3 class="text-lg font-semibold text-gray-800 border-b border-surface-300 pb-3">Imagen de Fondo</h3>
                <x-admin.form-file-upload name="img" label="Imagen (720x1280 recomendado)" :currentImage="$theme->img" />
            </div>
