// === RTCStream Player =========================================================
// Mayo-2025 · Versión 2.0.0 (Simplificada para URL completa)
//
...
(function () {
  ...
  window.RTCStreamPlayer = {
    ...
    init: function(options) {
      ...
      console.log('RTCStreamPlayer 2.0 inicializado con URL:', this.config.wsUrl);
      
      // Actualizar slug en la UI si existe el elemento
      if ($slugEl) {
        const slug = this.config.wsUrl ? this.config.wsUrl.split('/').pop() : '';
        $slugEl.textContent = slug || this.config.title;
      }
      
      // Autoplay si está configurado
      if (this.config.autoplay && this.config.wsUrl) {
  ...
  async function connect () {
    ...
    // Depuración exhaustiva para encontrar el problema
    console.log('RTCStreamPlayer config completo:', window.RTCStreamPlayer.config);
    console.log('wsUrl extraída:', wsUrl);
    
    // Verificar que la URL de WebSocket esté presente y tenga el formato correcto
    if (!wsUrl) {
    ...
    if (!wsUrl.match(/^wss?:\/\/[^\s]+:\d+\/ws\/listen\/[^\s]+$/)) {
      console.warn('Advertencia: El formato de la URL no coincide con el esperado (wss://host:puerto/ws/listen/slug)');
    }
    
    console.log('Conectando a WebSocket (URL exacta):', wsUrl);
    ...
    switch(m.type) {
      ...
      case 'webRtcTransportCreated':
        ...
        recvTransport.on('connect', ({dtlsParameters}, cb) => { 
          send('connectWebRtcTransport', {transportId: recvTransport.id, dtlsParameters}); 
          cb();
        });
      ...
      case 'error': 
        st('Error: ' + m.payload.message, 'error'); 
        scheduleRetry(); 
        break;
    }
  }
  ...
  async function createConsumer({id, producerId, kind, rtpParameters}) {
    try {
      ...
      st('Error creando reproductor', 'error');
      scheduleRetry();
    }
  }
  ...
  console.log('RTCStream Player v2.0.0 listo');
})();
